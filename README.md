...
View > Tab Style provides Soft Rounded (default), Slanted, Chamfered (the original shape), and Compact. Compact uses a low-height shape with a vertical left edge and subtle bottom-right skirt. The selection applies immediately to both main panels and Compare tabs, scales with the chosen font size, and is saved in `pfc.ini`.
