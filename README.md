...
F9 compares one selected item in the active panel and one in the next visible panel (or two items in the active panel); with no selection it compares those two current folders. File modes retain aligned text with source-accurate line numbers, CSV/TSV tables, binary hex with SHA-256, search, and two-second auto-refresh. Folder Compare scans in the background with Esc cancellation, Recursive, semicolon masks, optional content hashing, sortable Left only/Right only/Different/Identical/Left newer/Right newer results, difference filtering, and content Find/Case sensitive controls. Ctrl+Right, Ctrl+Left, and Space assign Copy →, ← Copy, or Skip to selected rows. `Dry Run & Sync` displays every planned operation and requires confirmation; v0.9 only copies selected directions through PFC's existing conflict/error engine and never automatically deletes files. Double-click or Enter on a paired file opens the existing detailed comparer.
