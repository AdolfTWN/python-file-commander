...
Current version: **v0.15.0 — Inline Workflow & Search Actions**
...
Copy, move, and clipboard paste detect name conflicts and offer Replace, Skip, Keep Both, Cancel, and Apply to All. Multi-item operations open a copyable result window listing exact failed paths with Retry Failed. The file context menu includes Run as Admin for supported Windows executables/scripts, ZIP compression, ZIP/7z extraction, and CMD/PowerShell shortcuts opened at the clicked location. Files > Continue After File Errors controls whether remaining items continue after a failure and defaults on. The Files menu groups clipboard operations, target-panel operations, rename/new folder, both delete modes, safety switches, Favorites, Recent Folders, Preview, Search, Compare, paths, and Exit.
