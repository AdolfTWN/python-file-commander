...
View > Tab Style provides Right Skirt (default), Rounded, and Squarish. Right Skirt uses a low-height shape with a vertical left edge and steep curved bottom-right skirt; Rounded curves only the top corners and keeps a square bottom. The selection applies immediately to both main panels and Compare tabs, scales with the chosen font size, and is saved in `pfc.ini`; legacy Compact settings migrate automatically to Right Skirt.
