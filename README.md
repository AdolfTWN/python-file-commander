...
Current core: two resizable panes, tabs, drive/path navigation, marked column sorting, multi-select, rename (F2), preview in the other panel (F3), recursive name search (F4), copy (F5), move (F6), new folder (F7), permanent delete (F8), and refresh (Ctrl+R). Right enters a folder, Left returns to its parent, and Ctrl+Up duplicates the current folder into a new tab.
