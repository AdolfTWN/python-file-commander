...
Right-click a local file or folder to open the native Windows File Explorer menu directly; Shift+F10 and the Menu key provide the same keyboard behavior. To open PFC's compact task menu, left-click an item and keep the pointer over that same row for two seconds. Open/preview, clipboard, target-panel transfer, rename, and both delete modes stay directly visible. Compare, Folder Space Analyzer, compression, and extraction live one level down under **Analyze & Archive**; Windows administrative, terminal, shortcut, and path actions live under **More Actions**. Moving the pointer, changing the selection, beginning a drag, or leaving the file list cancels the delayed menu. Archive workspaces use the PFC menu because their virtual items have no native Explorer path. F8 is unbound and reserved for future use.
