...
Right-click a file or folder for a compact, task-grouped menu. Open/preview, clipboard, target-panel transfer, rename, and both delete modes stay directly visible. Compare, Folder Space Analyzer, compression, and extraction live one level down under **Analyze & Archive**; Windows administrative, terminal, shortcut, path, and Explorer actions live under **More Actions**. Right-click preserves an existing multi-selection; Shift+F10 or the Menu key opens the same menu from the keyboard. Disabled entries clearly indicate operations that need one item, two items, a folder, Windows, or a local workspace.
