...
_SINGLE_PANEL_TRANSLATIONS = {
    ...
    "Sample tabs and file list; panel layout is shown below.": ("範例分頁與檔案清單；下方另顯示面板配置。", "示例标签与文件列表；下方另显示面板布局。", "탭과 파일 목록 예제입니다. 패널 배치는 아래에 표시됩니다."),
    "Syntax colors & Markdown (Extension Effect)": ("語法配色與 Markdown 呈現（Extension Effect）", "语法配色与 Markdown 渲染（Extension Effect）", "구문 색상 및 Markdown (Extension Effect)"),
    "Choose a style below. The comparison shows your current style and the draft; only Apply or OK changes PFC.": ("比較目前樣式與待套用樣式；下方調整，按「套用」或「確定」才會生效。", "比较当前样式与待应用样式；在下方调整，点击“应用”或“确定”才会生效。", "현재 스타일과 변경안을 비교하세요. 아래에서 선택하고 적용 또는 확인을 눌러 반영합니다."),
    "Compare tab style and panel layout before applying. Tab colors and locks stay in each tab’s right-click menu.": ("套用前比較分頁樣式與面板配置；分頁顏色及鎖定仍在各分頁右鍵選單。", "应用前比较标签样式与面板布局；标签颜色及锁定仍在各标签右键菜单。", "적용 전 탭 스타일과 패널 배치를 비교하세요. 탭 색상과 잠금은 각 탭의 우클릭 메뉴에 있습니다."),
    "Configure file menus, delete behavior and path shortcuts. Changes apply only after Apply or OK.": ("設定檔案選單、刪除方式與路徑捷徑；按「套用」或「確定」才會生效。", "设置文件菜单、删除方式与路径快捷方式；点击“应用”或“确定”才会生效。", "파일 메뉴, 삭제 동작과 경로 바로 가기를 설정합니다. 적용 또는 확인 후 반영됩니다."),
    "Controls F3 Preview only. It does not change files, index drives or enable external plugins.": ("僅控制 F3 預覽；不修改檔案、不索引磁碟，也不啟用外部外掛。", "仅控制 F3 预览；不修改文件、不索引磁盘，也不启用外部插件。", "F3 미리 보기에만 적용됩니다. 파일 수정, 드라이브 색인 또는 외부 플러그인 활성화를 하지 않습니다."),
    "Choose the interface language and Windows sign-in behavior. Updates remain a manual action in Help.": ("選擇介面語言與 Windows 登入啟動方式；更新仍在「說明」中手動執行。", "选择界面语言与 Windows 登录启动方式；更新仍在“帮助”中手动执行。", "인터페이스 언어와 Windows 로그인 동작을 선택합니다. 업데이트는 도움말에서 수동 실행합니다."),
    "Auto fits the main window width. Turn it off for a fixed percentage. Settings keeps a stable reading size.": ("自動模式配合主視窗寬度調整；關閉後指定固定倍率。設定視窗維持穩定字級。", "自动模式配合主窗口宽度调整；关闭后指定固定倍率。设置窗口保持稳定字号。", "자동 모드는 기본 창 너비에 맞춥니다. 끄면 고정 배율을 선택합니다. 설정 창 글자 크기는 유지됩니다."),
    "1 Panel: folder tree + file list with shared tabs. 2–4 Panels: separate file lists and tab groups.": ("1 面板：資料夾樹＋檔案清單，共用分頁。2–4 面板：各自的檔案清單與分頁組。", "1 面板：文件夹树＋文件列表，共享标签。2–4 面板：各自的文件列表与标签组。", "1 패널: 폴더 트리와 파일 목록, 공유 탭. 2–4 패널: 독립 파일 목록과 탭 그룹."),
    "GB is bold; TB is bold red. This changes display only, not file sizes.": ("GB 使用粗體；TB 使用紅色粗體。只改變顯示，不改變檔案大小。", "GB 使用粗体；TB 使用红色粗体。只改变显示，不改变文件大小。", "GB는 굵게, TB는 빨간 굵은 글씨로 표시합니다. 파일 크기는 변경되지 않습니다."),
    "File Explorer uses the Windows native file menu. Blank-area and column menus remain PFC shortcuts.": ("File Explorer 使用 Windows 原生檔案選單；空白區及欄位保留 PFC 快速選單。", "File Explorer 使用 Windows 原生文件菜单；空白区及列保留 PFC 快捷菜单。", "File Explorer는 Windows 기본 파일 메뉴를 사용합니다. 빈 영역과 열 메뉴는 PFC 바로 가기를 유지합니다."),
    "Continue with the remaining files after a failure; errors are still reported.": ("某個檔案失敗後繼續處理其餘檔案；仍會回報錯誤。", "某个文件失败后继续处理其余文件；仍会报告错误。", "실패 후 나머지 파일을 계속 처리하며 오류는 계속 보고합니다."),
    "On: syntax colors and formatted Markdown. Off: plain source text. Outline, properties, links and tables are in F3 Preview.": ("開啟：語法配色與 Markdown 排版。關閉：純原始文字。大綱、屬性、連結及表格工具位於 F3 預覽。", "开启：语法配色与 Markdown 排版。关闭：纯原始文本。大纲、属性、链接及表格工具位于 F3 预览。", "켜기: 구문 색상과 Markdown 서식. 끄기: 원본 텍스트. 목차, 속성, 링크 및 표 도구는 F3 미리 보기에 있습니다."),
    "Changes the interface after Apply, not your file names.": ("套用後變更介面語言，不會更改檔名。", "应用后更改界面语言，不会更改文件名。", "적용 후 인터페이스 언어를 변경하며 파일 이름은 바꾸지 않습니다."),
    "Compare at full size…": ("原尺寸並列比較…", "原尺寸并列比较…", "원본 크기로 비교…"),
    "Style details from demo screenshots; font size is shown separately.": ("示範截圖的樣式細節；字型大小另以文字範例呈現。", "示例截图的样式细节；字体大小另以文本示例呈现。", "데모 화면의 스타일 세부 비교입니다. 글꼴 크기는 별도 텍스트 예제로 표시합니다."),
    "File text after Apply · {percent}%": ("套用後的檔名文字大小 · {percent}%", "应用后的文件名文字大小 · {percent}%", "적용 후 파일명 글자 크기 · {percent}%"),
    "Auto font · {percent}% reference; follows window width": ("自動字級 · 以 {percent}% 示範，實際依視窗寬度調整", "自动字号 · 以 {percent}% 演示，实际依窗口宽度调整", "자동 글꼴 · {percent}% 예시, 실제 크기는 창 너비에 맞춤"),
    "Date column hidden; format is remembered.": ("日期欄位已隱藏；保留格式設定。", "日期列已隐藏；保留格式设置。", "날짜 열은 숨겨져 있으며 형식 설정은 유지됩니다."),
    "Delete moves items to Recycle Bin. Shift+Delete is always permanent.": ("Delete 移至資源回收筒；Shift+Delete 一律永久刪除。", "Delete 移至回收站；Shift+Delete 始终永久删除。", "Delete는 휴지통으로 이동하며 Shift+Delete는 항상 영구 삭제합니다."),
    "Warning: Delete will permanently remove files after Apply.": ("注意：套用後，Delete 將永久刪除檔案。", "注意：应用后，Delete 将永久删除文件。", "경고: 적용 후 Delete는 파일을 영구 삭제합니다."),
    ...
    'Appearance': ('外觀', '外观', '모양'),
    'Layout & Tabs': ('版面與分頁', '布局与标签页', '레이아웃 및 탭'),
    'Paths & Operations': ('路徑與操作', '路径与操作', '경로 및 작업'),
    'General': ('一般', '常规', '일반'),
    'Apply': ('套用', '应用', '적용'),
    'OK': ('確定', '确定', '확인'),
    'Changes take effect only after Apply or OK.': ('按「套用」或「確定」後才會生效。', '点击“应用”或“确定”后才会生效。', '적용 또는 확인을 눌러야 변경 사항이 반영됩니다.'),
    'Unsaved changes': ('有尚未套用的變更', '有尚未应用的更改', '적용하지 않은 변경 사항'),
    'No pending changes': ('沒有待套用的變更', '没有待应用的更改', '보류 중인 변경 없음'),
    'Name · current tab': ('檔名 · 目前分頁', '文件名 · 当前标签页', '이름 · 현재 탭'),
    'Name · all tabs': ('檔名 · 所有分頁', '文件名 · 所有标签页', '이름 · 모든 탭'),
    'Status icons': ('狀態圖示', '状态图标', '상태 아이콘'),
    'Show Ext column': ('顯示 Ext 欄位', '显示 Ext 列', 'Ext 열 표시'),
    'Show Size column': ('顯示 Size 欄位', '显示 Size 列', '크기 열 표시'),
    'Show Date Modified column': ('顯示修改日期欄位', '显示修改日期列', '수정 날짜 열 표시'),
    'Date format': ('日期格式', '日期格式', '날짜 형식'),
    'Time format': ('時間格式', '时间格式', '시간 형식'),
    'F3 Preview': ('F3 預覽', 'F3 预览', 'F3 미리 보기'),
    'Windows startup': ('Windows 啟動', 'Windows 启动', 'Windows 시작'),
    'Windows only. No system change is made on this platform.': ('僅適用 Windows，不會修改目前平台的系統設定。', '仅适用 Windows，不会修改当前平台的系统设置。', 'Windows 전용입니다. 이 플랫폼의 시스템 설정은 변경하지 않습니다.'),
    'Style comparison': ('樣式對照', '样式对比', '스타일 비교'),
    'Sample screenshots from PFC with demo files (100%). No personal folders are captured.': ('使用示範檔案的 PFC 實際截圖（100%）；不擷取私人資料夾。', '使用示例文件的 PFC 实际截图（100%）；不截取私人文件夹。', '샘플 파일을 사용한 PFC 실제 화면(100%)입니다. 개인 폴더는 캡처하지 않습니다.'),
    ...
    'Tabs per panel': ('各面板獨立分頁', '各面板独立标签页', '패널별 탭'),
    'Example': ('範例', '示例', '예시'),
    'Browse…': ('瀏覽…', '浏览…', '찾아보기…'),
    'Auto adjusts to window and panel widths. Turn it off to choose a fixed scale. The settings dialog keeps a stable reading size.': ('自動依視窗和面板寬度調整；關閉後可指定固定倍率。設定視窗維持穩定的閱讀字級。', '自动依窗口和面板宽度调整；关闭后可指定固定倍率。设置窗口保持稳定的阅读字号。', '창과 패널 너비에 자동으로 맞춥니다. 고정 배율을 선택하려면 끄세요. 설정 창의 글자 크기는 유지됩니다.'),
    '1 Panel combines a folder tree and file list with shared tabs. Tab colors and lock modes remain in each tab’s right-click menu.': ('單面板結合目錄樹、檔案清單和共用分頁。分頁顏色及鎖定模式仍可從各分頁右鍵設定。', '单面板结合目录树、文件列表和共享标签页。标签颜色及锁定模式仍可从各标签右键设置。', '1 패널은 폴더 트리, 파일 목록 및 공유 탭을 결합합니다. 탭 색상과 잠금은 각 탭의 오른쪽 클릭 메뉴에서 설정합니다.'),
    'Name visibility applies to the tab where Settings was opened. Other column preferences apply to all tabs. Hidden columns can be restored here.': ('檔名顯示選項僅影響開啟設定時的分頁；其他欄位設定影響所有分頁。隱藏的欄位可在此恢復。', '文件名显示选项仅影响打开设置时的标签页；其他列设置影响所有标签页。隐藏的列可在此恢复。', '이름 표시 옵션은 설정을 연 탭에 적용됩니다. 다른 열 설정은 모든 탭에 적용됩니다. 숨긴 열을 여기서 복원할 수 있습니다.'),
    'File Explorer uses the Windows native file menu. Blank-area and column menus remain PFC shortcuts. Disabling Recycle Bin makes Delete permanent; Shift+Delete always bypasses it.': ('File Explorer 使用 Windows 原生檔案選單；空白區和欄位保留 PFC 快速選單。關閉資源回收筒會永久刪除；Shift+Delete 一律永久刪除。', 'File Explorer 使用 Windows 原生文件菜单；空白区和列保留 PFC 快速菜单。关闭回收站会永久删除；Shift+Delete 始终永久删除。', 'File Explorer는 Windows 기본 파일 메뉴를 사용합니다. 빈 영역과 열 메뉴는 PFC 바로 가기입니다. 휴지통을 끄면 영구 삭제되며 Shift+Delete는 항상 휴지통을 건너뜁니다.'),
    'Extension Effect enables syntax colors and Markdown rendering in F3 Preview. Reading tools (outline, properties, links and tables) remain in Preview. Link resolution stays within the selected document workspace; this setting does not scan drives.': ('Extension Effect 啟用 F3 預覽的語法配色及 Markdown 呈現。大綱、屬性、連結及表格工具仍在預覽視窗。連結解析限於所選文件工作區，不會因切換此設定掃描磁碟。', 'Extension Effect 启用 F3 预览的语法配色及 Markdown 渲染。大纲、属性、链接及表格工具仍在预览窗口。链接解析限于所选文档工作区，不会因切换此设置扫描磁盘。', 'Extension Effect는 F3 미리 보기의 구문 색상과 Markdown 렌더링을 활성화합니다. 목차, 속성, 링크, 표 도구는 미리 보기에 있습니다. 링크는 선택한 문서 작업 공간에서만 해석하며 이 설정은 드라이브를 검색하지 않습니다.'),
    'Auto Start writes a Windows sign-in entry only when applied. Follow your organization’s software policy. Updates remain a manual action in Help.': ('自動啟動僅在套用時修改 Windows 登入啟動項目，請遵守組織軟體政策。更新仍需在說明選單手動執行。', '自动启动仅在应用时修改 Windows 登录启动项，请遵守组织软件政策。更新仍需在帮助菜单手动执行。', '자동 시작은 적용할 때만 Windows 로그인 항목을 변경합니다. 조직의 소프트웨어 정책을 따르세요. 업데이트는 도움말에서 수동 실행합니다.'),
    'Blue cloud: online only. Outlined green check: local copy. Filled green check: kept offline. Missing status is unknown, not proof of sync.': ('藍雲：僅線上；空心綠勾：本機副本；實心綠勾：保留離線。沒有狀態代表未知，不代表已同步。', '蓝云：仅在线；空心绿勾：本地副本；实心绿勾：保留离线。没有状态表示未知，不代表已同步。', '파란 구름: 온라인 전용. 빈 초록 체크: 로컬 복사본. 채운 초록 체크: 오프라인 유지. 상태 없음은 알 수 없음을 뜻하며 동기화 완료가 아닙니다.'),
...
_TRANSLATIONS = {
    ...
    "zh_TW": {
        ...
        **{key: labels[0] for key, labels in _MARKDOWN_READING_TRANSLATIONS.items()},
        ...
        "Added: Long Filename Scrolling reads the focused name without middle ellipses; View controls scrolling and Explorer/PFC right-click preferences.": "新增：長檔名跑馬燈完整顯示目前選取的名稱，不再省略中間文字；檢視選單可設定捲動與 Explorer／PFC 右鍵偏好。",
        "Changed: Removed the two-second dwell popup; leaving predefined folder prefixes restores the ROOT icon and full path, with vivid prefix icons inside matching folders.": "變更：移除停留兩秒彈出選單；離開預設路徑後還原 ROOT 圖示及完整路徑，符合前綴的資料夾使用鮮豔圖示。",
        "Fixed: Refresh preserves the focused file; corrupt ZIP files no longer raise an archive-menu counting thread exception.": "修正：刷新時保留檔案焦點；損壞 ZIP 不再造成壓縮檔選單計數執行緒例外。",
        "Long Filename Scrolling": "長檔名跑馬燈",
        "Right Click Menu": "右鍵選單", "File Explorer": "檔案總管",
        "Root folder": "根目錄",
        ...
        "Fixed: Path editing preserves invalid input and stays synchronized when navigating; archive breadcrumbs retain the original location.": "修正：無效路徑保留供修改，導覽時同步路徑顯示；壓縮檔階層路徑保留原始位置。",
        "Folder tree": "資料夾樹", "File tree": "檔案樹",
        "Edit path (Ctrl+L / F12)": "編輯完整路徑（Ctrl+L / F12）",
        ...
        "Adjusted: The PFC actions menu opens after a selected item remains under the pointer for two seconds.": "調整：選取項目並讓滑鼠停留在同一列兩秒後，才會開啟 PFC 功能選單。",
    ...
    "zh_CN": {
        ...
        **{key: labels[1] for key, labels in _MARKDOWN_READING_TRANSLATIONS.items()},
        ...
        "Added: Long Filename Scrolling reads the focused name without middle ellipses; View controls scrolling and Explorer/PFC right-click preferences.": "新增：长文件名滚动完整显示当前选中的名称，不再省略中间文字；视图菜单可设置滚动与 Explorer／PFC 右键偏好。",
        "Changed: Removed the two-second dwell popup; leaving predefined folder prefixes restores the ROOT icon and full path, with vivid prefix icons inside matching folders.": "变更：移除停留两秒弹出菜单；离开预设路径后还原 ROOT 图标及完整路径，符合前缀的文件夹使用鲜艳图标。",
        "Fixed: Refresh preserves the focused file; corrupt ZIP files no longer raise an archive-menu counting thread exception.": "修复：刷新时保留文件焦点；损坏 ZIP 不再造成压缩文件菜单计数线程异常。",
        "Long Filename Scrolling": "长文件名滚动",
        "Right Click Menu": "右键菜单", "File Explorer": "文件资源管理器",
        "Root folder": "根目录",
        ...
        "Fixed: Path editing preserves invalid input and stays synchronized when navigating; archive breadcrumbs retain the original location.": "修复：无效路径保留供修改，导航时同步路径显示；压缩文件层级路径保留原始位置。",
        "Folder tree": "文件夹树", "File tree": "文件树",
        "Edit path (Ctrl+L / F12)": "编辑完整路径（Ctrl+L / F12）",
        ...
        "Adjusted: The PFC actions menu opens after a selected item remains under the pointer for two seconds.": "调整：选择项目并让鼠标停留在同一行两秒后，才会打开 PFC 功能菜单。",
    ...
    "ko": {
        ...
        **{key: labels[2] for key, labels in _MARKDOWN_READING_TRANSLATIONS.items()},
        ...
        "Added: Long Filename Scrolling reads the focused name without middle ellipses; View controls scrolling and Explorer/PFC right-click preferences.": "추가: 긴 파일 이름을 중간 생략 없이 스크롤합니다. 보기 메뉴에서 스크롤과 Explorer/PFC 오른쪽 클릭 메뉴를 설정할 수 있습니다.",
        "Changed: Removed the two-second dwell popup; leaving predefined folder prefixes restores the ROOT icon and full path, with vivid prefix icons inside matching folders.": "변경: 2초 대기 팝업을 제거했습니다. 지정 경로 밖에서는 ROOT 아이콘과 전체 경로를 복원하고 해당 폴더 안에서는 선명한 아이콘을 표시합니다.",
        "Fixed: Refresh preserves the focused file; corrupt ZIP files no longer raise an archive-menu counting thread exception.": "수정: 새로 고침 시 파일 포커스를 유지하며 손상된 ZIP의 메뉴 항목 수 계산 스레드에서 예외가 발생하지 않습니다.",
        "Long Filename Scrolling": "긴 파일 이름 스크롤",
        "Right Click Menu": "오른쪽 클릭 메뉴", "File Explorer": "파일 탐색기",
        "Root folder": "루트 폴더",
        ...
        "Fixed: Path editing preserves invalid input and stays synchronized when navigating; archive breadcrumbs retain the original location.": "수정: 잘못된 경로 입력을 유지하고 탐색 시 경로 표시를 동기화하며, 압축 파일의 계층 경로는 원래 위치를 유지합니다.",
        "Folder tree": "폴더 트리", "File tree": "파일 트리",
        "Edit path (Ctrl+L / F12)": "전체 경로 편집 (Ctrl+L / F12)",
        ...
        "Adjusted: The PFC actions menu opens after a selected item remains under the pointer for two seconds.": "조정: 선택한 항목 위에 포인터를 2초 동안 두면 PFC 작업 메뉴가 열립니다.",
