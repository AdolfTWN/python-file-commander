...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory() as raw:
        ...
        try:
            ...
            app.header_popup.show(app.view_menu_button, context)
            app.update_idletasks(); app.update()
            ...
            assert len(root_arrows) == sum(context.type(i)=='cascade' for i in range(context.index('end')+1))
            ...
            assert len(huge_arrows) == len(root_arrows)
            app.header_popup.close_all()
            app.header_popup.show(app.view_menu_button, context)
            root = app.header_popup.popups[0]
